...
void SteppingAction::UserSteppingAction(const G4Step* theStep)
{
      ...
      "Transportation")
  {
    ...
    if ((edepStep > 0.) && (theVolume==targetVolume))
    {
      ...
      G4cout<<"Foi depositado "<<edepStep<< " no volume target, por isso vamos ver se houve um hit."<<G4endl;
      //  ----------------- EU -----------------------------------------
        
         CheckAndProcessHit(x,y,z,edepStep);
...
//  --------------------- EU --------------------------------------------------------------
//
G4bool SteppingAction::CheckAndProcessHit(G4double x,G4double y, G4double z,
    G4double edepStep)
{
int numMolec=0;
int numResi=0;
